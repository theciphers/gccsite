...
class Command(BaseCommand):
    help = (
        "Send (or re-send) an activation mail for the given username or email."
    )
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--yes',
            action='store_true',
            help="do NOT confirm, send email right away",
        )
        parser.add_argument(
            'username_or_email', help="the username or email to send"
        )
    ...
    def handle(self, *args, **options):
        ...
        if user.is_active and not options['no_check_active']:
            self.stderr.write(
                "User already active: {} / {}".format(
                    user.username, user.email
                )
            )
            return

        question = "Send activation mail to {} / {}? [y/n]".format(
            user.username, user.email
        )
