...
class UsersTest(
    tests.WithContestantMixin, tests.WithOrgaUserMixin, tests.ProloginTestCase
):
    def test_profile_visibility(self):
        response = self.client.get(
            reverse('users:profile', args=[self.contestant.id])
        )
    ...
    def test_edit_profile_permission(self):
        self.assertInvalidResponse(
            self.client.get(reverse('users:edit', args=[self.contestant.id]))
        )
