...
from form.models import AnswerTypes, Question
...
class Applicant(models.Model):
    ...
    user = models.ForeignKey(
        ...
        related_name='migrate_applicant_user',
    )
    edition = models.ForeignKey(
        Edition,
        on_delete=models.CASCADE,
        related_name='migrate_applicant_edition',
    )
    ...
    def participations_count(self):
        return Applicant.objects.filter(
            user=self.user, status=ApplicantStatusTypes.confirmed.value
        ).count()
